...
public class APVBeatDetector extends APVPlugin {
	 ...
	 * 44100, respectively, so that it is possible to switch into FREQ_ENERGY mode
	 * with meaningful values.
	 * 
	 */
	public APVBeatDetector(Main parent) {
		super(parent);
	 ...
	 * @related BeatDetect
	 */
	public APVBeatDetector(Main parent, int timeSize, float sampleRate) {
		super(parent);
	...
	/**
	 * Set the object to use the requested algorithm. If an invalid value is passed,
	 * the function will report and error and default to BeatDetect.SOUND_ENERGY
	 * 
	 * @param algo
	...
	/**
	 * Analyze the samples in <code>buffer</code>. This is a cumulative process, so
	...
	/**
	 * In frequency energy mode this returns the number of frequency bands currently
	 * being used. In sound energy mode this always returns 0.
	 * 
	...
	/**
	 * Returns the center frequency of the i<sup>th</sup> frequency band. In sound
	 * energy mode this always returns 0.
	 * 
	 * @param i
	...
	/**
	 * In frequency energy mode this returns true when a beat has been detect in the
	 * <code>i<sup>th</sup></code> frequency band. In sound energy mode this always
	 * returns false.
	 * 
	 * @param i
	 ...
	 * frequency range of a kick drum has been detected. This has been tuned to work
	 * well with dance / techno music and may not perform well with other styles of
	 * music. In sound energy mode this always returns false.
	 * 
	...
	/**
	 * In frequency energy mode this returns true if at least <code>threshold</code>
	 * bands of the bands included in the range <code>[low, high]</code> have
	 * registered a beat. In sound energy mode this always returns false.
	 * 
	 * @param low
	 ...
	 *            <code>[low, high]</code> that need to have registered a beat for
	 *            this to return true
	...
	/**
	 * Draws some debugging visuals in the passed PApplet. The visuals drawn when in
	 * frequency energy mode are a good way to determine what values to use with
	 * <code>inRange()</code> if the provided drum detecting functions aren't what
	 * you need or aren't working well.
	 * 
	 * @param p
